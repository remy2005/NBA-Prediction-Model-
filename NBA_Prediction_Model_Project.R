...
test_data$resi_h2h<-residuals(lm(headtohead ~ MMRdiff, data = test_data))
...
training_data$resi_h2h<-residuals(lm(headtohead ~ MMRdiff, data = training_data))
training_data$resi_gameresteddiff<-residuals(lm(gameresteddiff ~ ifHome, data= training_data))

creative_model<-glm(Win ~ ifHome + MMRdiff +resi_h2h
                    ,data=training_data, family = binomial, weights = Weight)

summary(creative_model)

prediction_c<- predict(creative_model, newdata = new_game, type = "response")
prediction_cw<-ifelse(prediction_c>0.5,1,0)
...
ptstotal_model<- lm(totalpts ~ MMRdiff*ifHome
                    , data = Cgamedf, weights = Weight)

predictptstotal<- predict(ptstotal_model, newdata = new_game, type = "response")
